library border_wall_mct;

{$DESCRIPTION 'Wall border generator plugin for MCTerra'}
...
begin
  //postmessage(app_hndl,WM_USER+307,-9,i);
...
begin
  generate_add_chunk(i,j,chunk);

  result:=true;  //vizov proshel uspeshno
end;
...
begin
  ...
  postmessage(app_hndl,WM_USER+309,integer(mess_to_manager),1);
...
begin        
  ...
  for i:=0 to length(blocks_ids)-1 do
  begin
    if blocks_ids[i].transparent then include(trans_bl,i);
    if blocks_ids[i].light_level>0 then include(light_bl,i);
    if blocks_ids[i].diffuse then include(diff_bl,i);
    if blocks_ids[i].solid then include(solid_bl,i);
  end;

  //dobavlaem v spisok na forme
  for i:=0 to length(blocks_ids)-1 do
...
begin
  ...
  plugin_settings.plugin_type:=169;  //border with auth
  plugin_settings.aditional_type:=1;  //border inside the map
  plugin_settings.full_name:='Wall border for MCTerra';
  plugin_settings.name:='Wall';
