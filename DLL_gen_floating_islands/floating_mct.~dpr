library floating_mct;

{$DESCRIPTION 'Floating islands generator plugin for MCTerra'}

{$R *.res}
{$R preview_floating.res}
...
uses
  ...
  ChunkProviderTest_u in 'ChunkProviderTest_u.pas',
...
begin
  app_hndl:=hndl;
  try
    init_form_settings(app_hndl);
    init_form_preview(app_hndl);
  except
    on e:exception do
    begin
...
begin
  ...
  plugin_settings.aditional_type:=2;  //floating landscape
  plugin_settings.full_name:='Skylands gen. for MCTerra';
  plugin_settings.name:='Floating Islands';
